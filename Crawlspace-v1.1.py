...
def set_app_icon(window):
    try:
        if sys.platform == "win32":
            # Windows can use .ico
            window.iconbitmap(resource_path("crawl.ico"))
        else:
            # macOS / Linux: use a PNG instead
            img = tk.PhotoImage(file=resource_path("crawl.png"))
            window.iconphoto(True, img)
            window._icon_ref = img  # keep reference
...
def display_slack_chat(file_path):
    ...
    set_app_icon(slack_chat_window)
